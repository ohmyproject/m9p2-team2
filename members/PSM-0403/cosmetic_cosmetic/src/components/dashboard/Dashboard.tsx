...
function getSearchIndexChangeParts(item: RankItem) {
  ...
  const arrow = diff >= 0 ? "▲" : "▼";
  return `${Math.round(previous)} → ${Math.round(current)} (${arrow}${Math.abs(diff)})`;
}
...
const NAVER_DATALAB_TOOLTIP = "해당 검색어가 검색된 횟수를 주/월별 각각 합산하여 조회기간 내 최다 검색량을 100으로 설정하여 상대적인 변화를 나타냅니다.\n출처: https://developers.naver.com/products/service-api/datalab/datalab.md";
        ...
        {trend.series.map((series, index) => {
          ...
          return (
            <span className="trend-legend-item" key={series.ingredient}>
              ...
              <IngredientTooltip label={series.ingredient}><span>{series.ingredient}</span></IngredientTooltip>
            </span>
          );
        })}
      </div>
...
export default function Dashboard() {
  ...
  const [activePage1Period, setActivePage1Period] = useState<Page1Period>("week");
  const [activePage1IngredientPeriod, setActivePage1IngredientPeriod] = useState<Page1Period>("week");
  ...
  async function handlePage1IngredientPeriodChange(period: Page1Period) {
    setActivePage1IngredientPeriod(period);
    setLoadState((current) => ({ ...current, dashboardSignals: "loading", dashboardSignalsError: "" }));
    try {
      const payload = await fetchCurrentApiDashboardSignals(period);
      setData((current) => ({
        ...
        page1: {
          ...current.page1,
          ingredientPopularity: payload.page1?.ingredientPopularity || current.page1.ingredientPopularity,
        },
      }));
      setLoadState((current) => ({ ...current, dashboardSignals: "ready", dashboardSignalsError: "" }));
    } catch (error) {
      setLoadState((current) => ({
        ...current,
        dashboardSignals: "error",
            ...
            <div className="dashboard-grid market-grid">
              <section className="card rank-card">
                ...
                <RankList
                  ...
                  valueLabel={activePage1Period === "month" ? "전월 대비 증감률" : "전주 대비 증감률"}
                  barMetric="growth"
                  valueMetric="growth"
              ...
              <section className="card rank-card">
                <div className="card-header">
                  ...
                  <div className="rank-source-toggle" aria-label="성분 인기 순위 데이터 기준 선택">
                    <TooltipButton
                      tooltip="네이버"
                      className={`period-button ${activeIngredientRankingSource === "naver" && activePage1IngredientPeriod === "week" ? "active" : ""}`}
                      onClick={() => { setActiveIngredientRankingSource("naver"); void handlePage1IngredientPeriodChange("week"); }}
                    >주간</TooltipButton>
                    <TooltipButton
                      tooltip="네이버"
                      className={`period-button ${activeIngredientRankingSource === "naver" && activePage1IngredientPeriod === "month" ? "active" : ""}`}
                      onClick={() => { setActiveIngredientRankingSource("naver"); void handlePage1IngredientPeriodChange("month"); }}
                    >월간</TooltipButton>
                    <button
                      type="button"
                ...
                <RankList
                  ...
                  valueLabel={activeIngredientRankingSource === "naver" ? (activePage1IngredientPeriod === "month" ? "전월 대비 증감률" : "전주 대비 증감률") : "평균 상품 순위"}
            ...
            <div className="compact-page-layout alert-page-layout">
              ...
              <div className="summary-strip alert-summary">
                <AlertSummaryChip
                  ...
                  tooltipDesc="수요 점수가 공급 점수보다 높아 기획 공백이 확인된 성분입니다. 1페이지 수요-공급 매트릭스 기준과 동일합니다."
                  matrixItems={data.page1.demandSupplyMatrix.filter((item) => item.status === "opportunity" || item.status === "shortage")}
                  matrixDemandColor="#3FA7B5"
                  matrixSupplyColor="#64748b"
                />
                <AlertSummaryChip
                  label="재고 리스크 성분"
                  ...
                  tooltipDesc="공급 점수가 수요 점수보다 높아 공급 과잉 리스크가 감지된 성분입니다. 1페이지 수요-공급 매트릭스 기준과 동일합니다."
                  matrixItems={data.page1.demandSupplyMatrix.filter((item) => item.status === "oversupply")}
                  matrixDemandColor="#64748b"
                  matrixSupplyColor="#C97A9B"
                />
                <AlertSummaryChip
                  label="부정 리뷰 이슈"
