...
type NaverComparePeriod = "week" | "month";
...
const NAVER_COMPARE_PERIOD_OPTIONS: Array<{ key: NaverComparePeriod; label: string }> = [
  { key: "week", label: "week" },
  { key: "month", label: "month" },
];

function getNaverCompareLabel(period: NaverComparePeriod) {
  return period === "week" ? "지난 주 대비 증감률" : "지난 달 대비 증감률";
}
...
const DATALAB_WEEKLY_API_REQUIRED_MESSAGE = "네이버 데이터랩 월간 검색 관심도를 불러오지 못했습니다.";
...
async function fetchCurrentApiDashboardSignals(comparePeriod: NaverComparePeriod = "month"): Promise<DashboardSignalsPayload> {
  const [summaryResult, weeklyResult, snapshotSummaryResult] = await Promise.allSettled([
    ...
    fetchLocalJson<DatalabWeeklyInterestResponse>(`/api/dashboard/datalab-weekly-interest?period=${comparePeriod}`),
  ...
  const weeklyPayload = normalizeDatalabWeeklyInterest(weekly, comparePeriod);
  ...
  if (weeklyResult.status === "rejected") {
    console.error("Naver DataLab 월간 검색 관심도 직접 조회 실패", weeklyResult.reason);
  }
  ...
  return {
    ...
    meta: {
      ...summaryPayload.meta,
      ...(weeklyPayload.meta || {}),
      dataRange: weeklyPayload.meta?.dataRange || summaryPayload.meta?.dataRange || "Naver DataLab 월간 검색 관심도",
      lastUpdated: weeklyPayload.meta?.lastUpdated || summaryPayload.meta?.lastUpdated || "-",
      comparisonLabel: getNaverCompareLabel(comparePeriod),
    },
    page1Summary: {
...
function normalizeDatalabWeeklyInterest(payload: DatalabWeeklyInterestResponse, comparePeriod: NaverComparePeriod = "month"): DashboardSignalsPayload {
  return {
    meta: {
      ...payload.meta,
      apiSource: payload.meta?.apiSource || payload.meta?.source || "naver_datalab",
      dataRange: payload.meta?.dataRange || "Naver DataLab 월간 검색 관심도",
...
function normalizeDashboardSummary(summary: DashboardSummaryResponse): DashboardSignalsPayload {
  ...
  return {
    meta: {
      ...
      comparisonLabel: summary.meta?.comparisonLabel || "지난 달 대비 증감률",
    },
    page1Summary: {
...
function normalizeDatalabWeeklyRankItems(items: unknown, sortBy: "growth" | "searchIndex", limit: number | null = 5): RankItem[] {
  ...
  items.forEach((item) => {
    ...
    if (currentWeekIndex === null || previousWeekIndex === null) {
      console.error("DataLab 월간 검색 관심도 응답에 이번 달/지난달 지수가 없습니다.", { label, row });
      return;
    }
...
function getCustomRankBarWidth(item: RankItem, items: RankItem[], valueGetter: (item: RankItem) => number) {
  const maxValue = Math.max(...items.map((row) => Math.max(0, Number(valueGetter(row) || 0))), 1);
  return Math.max(8, Math.min(100, (Math.max(0, Number(valueGetter(item) || 0)) / maxValue) * 100));
}
...
function buildPage2DecisionInsights(page2: DashboardData["page2"], marketProducts: MarketProduct[]) {
  const insights: string[] = [];
  const searchTrendSeries = page2.searchTrend?.series || [];
  const topCurrentSeries = searchTrendSeries
    .slice()
    .sort((a, b) => Number(b.values.at(-1) || 0) - Number(a.values.at(-1) || 0))[0];
  const topGrowthSeries = searchTrendSeries
...
function SummaryStrip({ summary, page1 }: { summary: Page1Summary; page1: DashboardData["page1"] }) {
  const [expandedSummary, setExpandedSummary] = useState<"rising" | "risk" | null>(null);
  const marketInterestTooltip =
    "산출근거: 기능/성분 TOP5의 네이버 검색 관심도 지수를 최근 선택 기간과 직전 기간으로 비교해 평균 증감률을 계산했습니다. 네이버 DataLab은 조회기간 내 최다 검색량을 100으로 설정한 상대 지수를 제공합니다.";
  const risingCandidates = (page1.ingredientPopularity || [])
    .filter((item) => Number(item.growth || 0) >= 0)
    .slice(0, Math.max(3, summary.risingIngredientCount || 3));
  const riskCandidates = (page1.ingredientPopularity || [])
    .filter((item) => Number(item.growth || 0) < 0)
    .slice(0, Math.max(1, summary.supplyShortageIngredientCount || 1));
  const items: Array<{
    label: string;
    value: string;
    icon: string;
    tone: string;
    tooltip?: string;
    detailKey?: "rising" | "risk";
    detailTitle?: string;
    detailBody?: string;
    detailItems?: RankItem[];
  }> = [
    { label: "분석 제품 수", value: `${formatNumber(summary.analyzedProducts)}개`, icon: "▯", tone: "" },
    { label: "분석 성분 수", value: `${formatNumber(summary.analyzedIngredients)}개`, icon: "◎", tone: "green" },
    { label: "시장 검색 관심도", value: formatPct(summary.totalSearchGrowthRate), icon: "▴", tone: "positive", tooltip: marketInterestTooltip },
    {
      label: "신제품 기획 후보",
      value: `${formatNumber(summary.risingIngredientCount)}개`,
      icon: "⚡",
      tone: "blue",
      detailKey: "rising" as const,
      detailTitle: "신제품 기획 후보 산출 기준",
      detailBody: "네이버 검색 관심도 상승률이 높고 시장 수요 신호가 확인된 성분을 후보로 집계했습니다.",
      detailItems: risingCandidates,
    },
    {
      label: "재고 리스크 성분",
      value: `${formatNumber(summary.supplyShortageIngredientCount)}개`,
      icon: "△",
      tone: "warning",
      detailKey: "risk" as const,
      detailTitle: "재고 리스크 성분 산출 기준",
      detailBody: "검색 관심도 하락 또는 수급 불균형 가능성이 있는 성분을 재고 리스크 후보로 집계했습니다.",
      detailItems: riskCandidates,
    },
  ];
  ...
  return (
    <div className="summary-strip">
      {items.map((item) => {
        const isDetailOpen = item.detailKey ? expandedSummary === item.detailKey : false;

        return (
          <div
            className={`summary-chip ${item.tone} ${item.tooltip ? "summary-tooltip" : ""} ${item.detailKey ? "summary-expandable" : ""}`}
          ...
          >
            ...
            {item.detailKey ? (
              <button
                ...
                type="button"
                aria-label={`${item.label} 세부내용 보기`}
                aria-expanded={isDetailOpen}
                onClick={() => setExpandedSummary(isDetailOpen ? null : item.detailKey)}
              >
                ⓘ
              </button>
            ) : null}
            {item.detailKey && isDetailOpen ? (
              <div className="summary-detail-panel">
                <strong>{item.detailTitle}</strong>
                <p>{item.detailBody}</p>
                {item.detailItems?.length ? (
                  <ul className="summary-detail-list">
                    {item.detailItems.map((detailItem, detailIndex) => (
                      <li key={`${item.detailKey}-${detailItem.label}-${detailIndex}`}>
                        <span>{detailItem.label}</span>
                        <em>{formatPct(detailItem.growth)} · {formatNumber(Math.round(Number(detailItem.previousWeekIndex || 0)))} → {formatNumber(Math.round(Number(detailItem.currentWeekIndex || detailItem.searchIndex || 0)))}</em>
                      </li>
                    ))}
                  </ul>
                ) : (
                  <p className="summary-detail-empty">현재 표시할 세부 성분 데이터가 없습니다.</p>
                )}
              </div>
            ) : null}
          </div>
...
function getSearchIndexChangeParts(item: RankItem) {
  ...
  const previousText = Number.isFinite(previous) ? formatNumber(Math.round(previous)) : "-";
  const currentText = Number.isFinite(current) ? formatNumber(Math.round(current)) : "-";
  const diffText = Number.isFinite(diff) ? formatNumber(Math.abs(Math.round(diff))) : "-";
  const direction = diff < 0 ? "▼" : "▲";

  return {
    rangeText: `${previousText} → ${currentText}`,
    diffBadge: `(${direction}${diffText})`,
  };
}
...
function RankList({
  ...
  sortMetric?: (item: RankItem) => number;
  ...
  const getSortValue = (item: RankItem) => {
    if (sortMetric) return Number(sortMetric(item) || 0);
    if (valueFormatter) return Number(item.averageRank ?? item.searchIndex ?? 0);
    return Number(item[valueMetric] || 0);
  };
  ...
  return (
    <>
      <div className="rank-list">
        ...
        const isGrowthMetric = !valueFormatter && valueMetric === "growth";
        const valueClassName = isGrowthMetric ? (growth < 0 ? "negative" : "positive") : "";
        const changeParts = isGrowthMetric ? getSearchIndexChangeParts(item) : null;
          ...
          <div className="rank-item" key={`${item.label}-${index}`}>
            ...
            <div className="rank-body">
              ...
              <div className="rank-bar">
                <span style={{ width: `${barValue ? getCustomRankBarWidth(item, sortedItems, barValue) : getRankBarWidth(item, sortedItems, barMetric)}%` }} />
              </div>
            </div>
            <div className={`rank-value ${valueClassName}`}>
              <strong>
                {valueFormatter
                  ? valueFormatter(item)
                  : valueMetric === "searchIndex"
                    ? formatNumber(Math.round(Number(item.searchIndex || 0)))
                    : formatPct(item.growth)}
              </strong>
              {!hideValueLabel ? <span>{changeParts ? `${changeParts.rangeText} ${changeParts.diffBadge}` : valueLabel}</span> : null}
            </div>
          </div>
        );
        })}
...
function RankSideControls({
  ...
  source?: IngredientRankingSource;
  activePeriod: NaverComparePeriod;
  activeSource?: IngredientRankingSource;
  onSourceChange?: (source: IngredientRankingSource) => void;
  onPeriodChange: (period: NaverComparePeriod) => void;
}) {
  const showSourceButtons = Boolean(activeSource && onSourceChange);

  return (
    <div className="rank-side-controls" aria-label="순위 데이터 기준 선택">
      {showSourceButtons ? (
        <div className="rank-side-group">
          {INGREDIENT_RANKING_SOURCE_OPTIONS.map((option) => (
            <button
              key={option.key}
              type="button"
              title={option.label}
              className={`side-icon-button source-${option.key} ${activeSource === option.key ? "active" : ""}`}
              onClick={() => onSourceChange?.(option.key)}
            >
              {option.key === "naver" ? "N" : "O"}
            </button>
          ))}
        </div>
      ) : null}

      {(!showSourceButtons || activeSource === "naver") ? (
        <div className="rank-side-group period-group">
          {NAVER_COMPARE_PERIOD_OPTIONS.map((option) => (
            <button
              key={option.key}
              type="button"
              className={`side-period-button ${activePeriod === option.key ? "active" : ""}`}
...
type IngredientTopMode = "naver-week" | "naver-month" | "oliveyoung";

function FunctionTopSwitch({
  activePeriod,
  onChange,
}: {
  activePeriod: NaverComparePeriod;
  onChange: (period: NaverComparePeriod) => void;
}) {
  return (
    <div className="top-switch two-state" aria-label="기능 TOP5 기간 선택">
      <button
        type="button"
        title="주 단위 자료"
...
function IngredientTopSwitch({
  ...
  activeSource: IngredientRankingSource;
  activePeriod: NaverComparePeriod;
  onChange: (mode: IngredientTopMode) => void;
}) {
  const activeMode: IngredientTopMode = activeSource === "oliveyoung"
    ? "oliveyoung"
    : activePeriod === "week"
      ? "naver-week"
      : "naver-month";
  ...
  return (
    <div className="top-switch three-state" aria-label="성분 TOP5 데이터 기준 선택">
      <button
        type="button"
        title="네이버 DataLab 주 단위 자료"
        className={activeMode === "naver-week" ? "active" : ""}
        onClick={() => onChange("naver-week")}
      >
        N·W
      </button>
      <button
        type="button"
        title="네이버 DataLab 월 단위 자료"
        className={activeMode === "naver-month" ? "active" : ""}
        onClick={() => onChange("naver-month")}
      >
        N·M
      </button>
      <button
        type="button"
        title="올리브영 평균 상품 순위"
        className={activeMode === "oliveyoung" ? "active olive" : ""}
        onClick={() => onChange("oliveyoung")}
      >
        O
      </button>
    </div>
...
export default function Dashboard() {
  ...
  const [activeFunctionComparePeriod, setActiveFunctionComparePeriod] = useState<NaverComparePeriod>("month");
  const [activeIngredientComparePeriod, setActiveIngredientComparePeriod] = useState<NaverComparePeriod>("month");
  ...
  async function loadDashboardSignals(
    comparePeriod: NaverComparePeriod = activeIngredientComparePeriod,
    target: "all" | "function" | "ingredient" = "all",
  ) {
    setLoadState((current) => ({ ...current, dashboardSignals: "loading", dashboardSignalsError: "" }));
    try {
      const payload = await fetchCurrentApiDashboardSignals(comparePeriod);
      const hasDatalabData = Boolean(payload.page1?.functionRisers?.length || payload.page1?.ingredientPopularity?.length);
      ...
      setData((current) => ({
        ...
        page1: {
          ...current.page1,
          ...(target === "all" || target === "function" ? { functionRisers: payload.page1?.functionRisers || current.page1.functionRisers } : {}),
          ...(target === "all" || target === "ingredient" ? { ingredientPopularity: payload.page1?.ingredientPopularity || current.page1.ingredientPopularity } : {}),
          functionDemand: payload.page1?.functionDemand || current.page1.functionDemand,
          ingredientDemand: payload.page1?.ingredientDemand || current.page1.ingredientDemand,
          insights: payload.page1?.insights || current.page1.insights,
          priceDistribution: current.page1.priceDistribution,
          demandSupplyMatrix: current.page1.demandSupplyMatrix,
        },
        page2: {
          ...current.page2,
          ...(payload.page2 || {}),
          searchTrend: payload.page2?.searchTrend || current.page2.searchTrend,
          concernMetrics: payload.page2?.concernMetrics || current.page2.concernMetrics,
          concernTable: payload.page2?.concernTable || current.page2.concernTable,
          marketProducts: payload.page2?.marketProducts || current.page2.marketProducts,
          insights: payload.page2?.insights || current.page2.insights,
        },
        page3: current.page3,
        page4: current.page4,
        page5: current.page5,
      ...
      setLoadState((current) => ({
        ...current,
        dashboardSignals: hasDatalabData ? "ready" : "error",
        dashboardSignalsError: hasDatalabData ? "" : DATALAB_WEEKLY_API_REQUIRED_MESSAGE,
      }));
    } catch (error) {
      ...
      setLoadState((current) => ({
        ...
        dashboardSignalsError: error instanceof Error ? error.message : DATALAB_WEEKLY_API_REQUIRED_MESSAGE,
      }));
    }
  }
  ...
  async function loadOliveYoungIngredientPopularity() {
    ...
    try {
      const rows = await fetchOliveYoungIngredientPopularityFromSupabase();
      setOliveYoungIngredientPopularity(rows);
      setOliveYoungRankingState({ status: "ready", error: "" });
    } catch (error) {
      console.error("올리브영 성분 인기 순위 조회 실패", error);
      setOliveYoungIngredientPopularity([]);
      setOliveYoungRankingState({
        status: "error",
        error: error instanceof Error ? error.message : "올리브영 성분 인기 순위를 불러오지 못했습니다.",
      });
    }
  }
  ...
  async function refreshDatalabData(periodKey = activeSearchPeriodKey) {
    await loadDashboardSignals(activeIngredientComparePeriod);
    await loadIngredientTrend(periodKey);
  }

  async function handleFunctionComparePeriodChange(periodKey: NaverComparePeriod) {
    setActiveFunctionComparePeriod(periodKey);
    await loadDashboardSignals(periodKey, "function");
  }

  async function handleIngredientComparePeriodChange(periodKey: NaverComparePeriod) {
    setActiveIngredientComparePeriod(periodKey);
    await loadDashboardSignals(periodKey, "ingredient");
  }

  async function handleIngredientTopModeChange(mode: IngredientTopMode) {
    if (mode === "oliveyoung") {
      setActiveIngredientRankingSource("oliveyoung");
      if (oliveYoungRankingState.status === "idle") {
        await loadOliveYoungIngredientPopularity();
      }
      return;
    }

    const periodKey = mode === "naver-week" ? "week" : "month";
    setActiveIngredientRankingSource("naver");
    setActiveIngredientComparePeriod(periodKey);
    await loadDashboardSignals(periodKey, "ingredient");
  }
  ...
  useEffect(() => {
    ...
    void loadDashboardSignals("month", "all");
    void loadIngredientTrend("snapshot");
          ...
          <article className={`panel ${activeTab === "A" ? "active" : ""}`}>
            ...
            <div className="dashboard-grid market-grid">
              <section className="card rank-card">
                <div className="card-header rank-card-top-header">
                  <span>기능(급상승 순위) TOP 5</span>
                  <FunctionTopSwitch
                    activePeriod={activeFunctionComparePeriod}
                    onChange={(periodKey) => void handleFunctionComparePeriodChange(periodKey)}
                  />
                </div>
                <RankList
                  items={data.page1.functionRisers}
                  valueLabel={getNaverCompareLabel(activeFunctionComparePeriod)}
                  barMetric="growth"
                  valueMetric="growth"
                ...
                />
                <div className="rank-standard-note rank-standard-tooltip" data-tooltip="해당 검색어가 검색된 횟수를 주/월별 각각 합산하여 조회기간 내 최다 검색량을\n100으로 설정하여 상대적인 변화를 나타냅니다.\n출처 : https://developers.naver.com/products/service-api/datalab/datalab.md">* 네이버 검색 관심도 기준</div>
              </section>
              ...
              <section className="card rank-card">
                <div className="card-header rank-card-top-header">
                  ...
                  <IngredientTopSwitch
                    activeSource={activeIngredientRankingSource}
                    activePeriod={activeIngredientComparePeriod}
                    onChange={(mode) => void handleIngredientTopModeChange(mode)}
                  />
                </div>
                <RankList
                  items={ingredientPopularityItems}
                  valueLabel={activeIngredientRankingSource === "naver" ? getNaverCompareLabel(activeIngredientComparePeriod) : "평균 상품 순위"}
                  barMetric={activeIngredientRankingSource === "naver" ? "growth" : "searchIndex"}
                  ...
                  valueFormatter={activeIngredientRankingSource === "oliveyoung"
                    ...
                    : undefined}
                  sortMetric={activeIngredientRankingSource === "oliveyoung"
                    ? (item) => Number(item.averageRank || 0)
                    : (item) => Number(item.growth || 0)}
                  sortDirection="desc"
                  barValue={activeIngredientRankingSource === "oliveyoung"
                    ? (item) => Number(item.averageRank || 0)
                    : undefined}
                  hideValueLabel={activeIngredientRankingSource === "oliveyoung"}
                ...
                />
                <div
                  className={`rank-standard-note ${activeIngredientRankingSource === "naver" ? "rank-standard-tooltip" : ""}`}
                  data-tooltip={activeIngredientRankingSource === "naver" ? "해당 검색어가 검색된 횟수를 주/월별 각각 합산하여 조회기간 내 최다 검색량을\n100으로 설정하여 상대적인 변화를 나타냅니다.\n출처 : https://developers.naver.com/products/service-api/datalab/datalab.md" : undefined}
                >
                  * {activeIngredientRankingSource === "naver" ? "네이버 검색 관심도 기준" : "평균 상품 순위"}
                </div>
              </section>
