...
export async function GET(request: Request) {
  try {
    ...
    const comparePeriod = resolveComparePeriod(url.searchParams.get("period"));
    ...
    const { startDate, endDate, timeUnit, comparisonLabel } = getPage1ComparisonRange(comparePeriod);
    const [functionRows, ingredientRows] = await Promise.all([
      fetchWeeklyInterestRows(FUNCTION_SIGNAL_GROUPS, startDate, endDate, clientId, clientSecret, timeUnit),
      fetchWeeklyInterestRows(buildIngredientKeywordGroups(), startDate, endDate, clientId, clientSecret, timeUnit),
    ]);
...
function resolveComparePeriod(value: string | null): "week" | "month" {
  return value === "week" ? "week" : "month";
}

function getPage1ComparisonRange(period: "week" | "month") {
  if (process.env.DASHBOARD_START_DATE && process.env.DASHBOARD_END_DATE) {
    return {
      ...
      timeUnit: period === "week" ? "week" as DatalabTimeUnit : "month" as DatalabTimeUnit,
      comparisonLabel: period === "week" ? "지난 주 대비 증감률" : "지난 달 대비 증감률",
    };
  }
  ...
  if (period === "week") {
    // 최근 8주와 직전 8주를 주 단위로 합산 비교합니다.
    start.setDate(start.getDate() - 111);
    return {
      startDate: toDateString(start),
      endDate: toDateString(end),
      timeUnit: "week" as DatalabTimeUnit,
      comparisonLabel: "지난 주 대비 증감률",
    };
  }

  // 최근 2개월과 직전 2개월을 월 단위로 합산 비교합니다.
  start.setMonth(start.getMonth() - 3);

  return {
    startDate: toDateString(start),
    endDate: toDateString(end),
    timeUnit: "month" as DatalabTimeUnit,
    comparisonLabel: "지난 달 대비 증감률",
  };
}
