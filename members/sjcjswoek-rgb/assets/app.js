(function () {
  ...
  function renderAgentPage() {
      ...
      const recommendations = qs("#aiRecommendations");
      const suggestions = qs("#agentSuggestions");
      const targetStrategy = qs("#targetStrategy");
  
      if (!input || !recommendations || !suggestions || !targetStrategy) {
        console.warn("AI Agent HTML 요소 누락");
        return;
      }
      ...
      suggestions.innerHTML = defaultSuggestions.map((suggestion) => `
      ...
      const strategyMap = {
        md: {
          title: "MD용 상품 기획서 초안",
          level: "높음",
          summary: "성분 트렌드, 검색 관심도, 리뷰 이슈, 수요-공급 신호를 바탕으로 MD가 빠르게 검토할 수 있는 상품 기획서 초안을 생성했습니다.",
          evidence: "핵심 후보 성분: 레티놀 / 리뷰 개선 이슈: 자극감·건조감 / 급상승 성분 수: 12개",
          strategy: "레티놀을 중심 성분으로 설정하고, 자극감·건조감 이슈를 보완하는 콘셉트·타겟·판매 전략을 구성합니다.",
          targetTitle: "MD 상품 기획서 구성",
          issues: ["시장 트렌드 기반 성분 선정", "리뷰 불만 개선 포인트 반영", "판매 채널용 차별화 메시지 필요"],
          directions: [
            "[상품 콘셉트] 레티놀 기반 저자극 기능성 케어 제품",
            "[타겟 고객] 성분 효능은 원하지만 자극감·사용감에 민감한 소비자",
            "[핵심 효능] 안티에이징 + 장벽 케어 + 저자극 사용감",
            "[보완 성분] 세라마이드·판테놀·히알루론산 조합 검토",
            "[가격/용량] 경쟁 제품의 10ml당 가격 분포를 참고해 중간 가격대 진입 검토",
            "[판매 채널] 올리브영/자사몰/SNS 숏폼 콘텐츠 연계 가능",
          ],
          actions: [
            "[제품명 예시] 데일리 저자극 레티놀 장벽 세럼",
            "[상세페이지 메시지] 검색량 상승 성분 + 리뷰 기반 개선 포인트를 카드형 콘텐츠로 배치",
            "[마케팅 카피] 효과는 선명하게, 사용감은 편안하게",
        ...
        entry: {
          title: "이번 주 진입 검토 성분 추천",
          level: "높음",
          summary: "이번 주는 검색 관심도 상승과 공급 기회가 함께 보이는 성분을 우선 검토하는 것이 좋습니다.",
          evidence: "전체 검색 관심도 +18.4%, 급상승 성분 12개, 공급 부족 성분 7개",
          strategy: "PDRN·레티놀·병풀/시카 중 수요 상승 대비 공급이 낮은 후보를 먼저 검토합니다.",
          targetTitle: "신규 진입 검토 전략",
          issues: ["검색 관심 증가", "공급 기회", "차별화 필요"],
          directions: ["수요 상승 대비 제품 수가 낮은 성분을 우선 후보로 설정합니다.", "성분 효능을 직관적으로 보여주는 상세페이지 메시지를 구성합니다.", "기존 인기 성분과 조합해 진입 장벽을 낮춥니다."],
          actions: ["후보 성분별 경쟁 제품 가격대와 리뷰 이슈를 비교합니다.", "상위 성분 조합 기준으로 상세페이지 키워드를 설계합니다.", "초기 리뷰 확보를 위한 체험단 전략을 병행합니다."],
        },
        dryRetinol: {
          title: "건성 타겟 레티놀 보틀 전략",
          level: "높음",
          summary: "검색 수요는 상승 중이지만 건성 리뷰에서 건조감 이슈가 함께 증가했습니다.",
          evidence: "레티놀 검색량 전주 대비 +24.8%, 건성 리뷰의 건조감 이슈 +9.4%",
          strategy: "레티놀 + 세라마이드/판테놀 보습 조합을 묶고, 첫 2주 사용 루틴을 상세페이지 상단에 배치하세요.",
          targetTitle: "레티놀 × 건성 타깃 전략",
          issues: ["건조감", "자극", "따가움"],
          directions: ["세라마이드/판테놀 결합", "저자극 메시지", "샘플링 유도"],
          actions: ["상세페이지 문구 개선", "리뷰 키워드 반영", "배너 카피 제안"],
        },
        oversupply: {
          title: "공급 과열 성분 대응 전략",
          level: "중간",
          summary: "공급 지수가 높은 성분은 신규 진입보다 차별화 메시지와 SKU 정리가 우선입니다.",
          evidence: "동기간 일부 성분의 공급 지수는 높지만 검색 수요 지수는 상대적으로 낮은 구간에 있습니다.",
          strategy: "동일 성분 중심의 단순 진입은 피하고, 제형·피부 타입·사용 상황을 좁혀 포지셔닝합니다.",
          targetTitle: "공급 과열 대응 카드",
          issues: ["제품 수 증가", "차별화 약화", "가격 경쟁 가능성"],
          directions: ["단일 성분 소구 지양", "피부 타입별 세분화", "가격보다 사용감 개선 강조"],
          actions: ["경쟁 제품 가격·리뷰 비교", "SKU 리뉴얼 검토", "상세페이지 차별화 문구 정리"],
        },
        review: {
          title: "부정 리뷰 기반 개선 전략",
          level: "중간",
          summary: "반복되는 부정 리뷰를 제품 개선 포인트와 상세페이지 보완 메시지로 전환해야 합니다.",
          evidence: "주요 부정 키워드 후보: 자극감·건조감·사용감 불만",
          strategy: "부정 리뷰를 FAQ, 사용법 안내, 보완 성분 메시지로 선제 대응합니다.",
          targetTitle: "리뷰 이슈 개선 액션 플랜",
          issues: ["자극감", "사용감 불만", "기대 효능 불일치"],
          directions: ["불만 키워드 우선순위화", "사용법 안내 강화", "긍정 리뷰 키워드 함께 활용"],
          actions: ["부정 키워드 TOP5 정리", "상세페이지 주의사항 추가", "FAQ에서 반복 이슈 해소"],
        },
        twentiesWhitening: {
          title: "20대 타겟 미백 상품 기획안",
          level: "높음",
          summary: "20대 타겟은 톤 개선, 잡티 예방, 가벼운 제형, 저자극 메시지에 반응할 가능성이 높습니다.",
          evidence: "연령대별 피부 고민과 성분 관심도 기반",
          strategy: "나이아신아마이드·비타민C·판테놀 조합으로 톤 개선과 저자극 메시지를 함께 제안합니다.",
          targetTitle: "20대 미백/톤 케어 캠페인 전략",
          issues: ["잡티/톤 고민", "가벼운 사용감 선호", "자극 우려"],
          directions: ["톤업·맑은 피부 표현 중심 메시지", "나이아신아마이드 조합 강조", "아침 루틴용 산뜻한 제형 소구"],
          actions: ["20대 타겟 광고 소재 제작", "사용 전후 기대감 카드 콘텐츠 제작", "민감 피부용 저자극 문구 추가"],
        },
      };
  
      function pickStrategy(question) {
        const normalized = (question || "").replace(/\s/g, "");
        if (normalized.includes("20대") || normalized.includes("미백") || normalized.includes("톤")) return strategyMap.twentiesWhitening;
        if (normalized.includes("건성") || normalized.includes("레티놀")) return strategyMap.dryRetinol;
        if (normalized.includes("공급") || normalized.includes("과열") || normalized.includes("포화")) return strategyMap.oversupply;
        if (normalized.includes("부정") || normalized.includes("리뷰") || normalized.includes("개선")) return strategyMap.review;
        if (normalized.includes("기획") || normalized.includes("MD") || normalized.includes("상품")) return strategyMap.md;
        return strategyMap.entry;
      }
  
      function renderCard(result) {
        recommendations.innerHTML = `
          <article class="recommendation-card">
            <div class="recommendation-top"><strong>${escapeHtml(result.title)}</strong><span>${escapeHtml(result.level)}</span></div>
            <p class="recommendation-overview">${escapeHtml(result.summary)}</p>
            <div class="recommendation-details">
              <div class="action-box"><span>근거 데이터</span><strong>${escapeHtml(result.evidence)}</strong></div>
              <div class="action-box"><span>추천 전략</span><strong>${escapeHtml(result.strategy)}</strong></div>
            </div>
          </article>
          ${result.downloadable ? `<button id="downloadPlanButton" class="btn btn-primary plan-download-button" type="button">📄 Word 기획서 다운로드</button>` : ""}
        `;
  
        targetStrategy.innerHTML = `
          <div class="target-strategy">
            <h2>${escapeHtml(result.targetTitle)}</h2>
            <div class="strategy-block"><span>핵심 이슈</span><div class="strategy-tags">${result.issues.map((item) => `<em>${escapeHtml(item)}</em>`).join("")}</div></div>
            <div class="strategy-block"><span>제안 방향</span><ul>${result.directions.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul></div>
            <div class="strategy-block"><span>추천 액션</span><ul>${result.actions.map((item) => `<li>${escapeHtml(item)}</li>`).join("")}</ul></div>
          </div>
        `;
  
        const downloadButton = qs("#downloadPlanButton");
        if (downloadButton) {
          downloadButton.onclick = () => {
            const content = `MD 상품 기획서 초안\n\n1. 기획 제목\n${result.title}\n\n2. 기획 배경\n${result.summary}\n\n3. 근거 데이터\n${result.evidence}\n\n4. 추천 전략\n${result.strategy}\n\n5. 핵심 이슈\n- ${result.issues.join("\n- ")}\n\n6. 제안 방향\n- ${result.directions.join("\n- ")}\n\n7. 실행 액션\n- ${result.actions.join("\n- ")}`;
            const blob = new Blob([content], { type: "application/msword;charset=utf-8" });
            const url = URL.createObjectURL(blob);
            const link = document.createElement("a");
            link.href = url;
            link.download = "MD_상품_기획서.doc";
            link.click();
      ...
      function showLoading(message) {
        recommendations.innerHTML = `<article class="recommendation-card"><div class="recommendation-top"><strong>AI가 데이터를 분석 중입니다...</strong><span>분석</span></div><p class="recommendation-overview">${escapeHtml(message)}</p></article>`;
        targetStrategy.innerHTML = "";
      }
  
      function runAgent(question) {
        const prompt = question || "이번 주 진입 검토 성분 추천해줘";
        input.value = prompt;
        showLoading("검색 신호와 리뷰 데이터를 종합 분석하고 있어요.");
        window.setTimeout(() => renderCard(pickStrategy(prompt)), 450);
      }
  
      qsa(".question-chip").forEach((chip) => { chip.onclick = () => runAgent(chip.textContent.trim()); });
  
      input.addEventListener("keydown", (event) => {
        if (event.key !== "Enter") return;
        event.preventDefault();
        runAgent(input.value || "이번 주 진입 검토 성분 추천해줘");
      });
      ...
      renderCard(strategyMap.dryRetinol);
    }
  ...
  function bindNavigation() {
    qsa(".nav-item").forEach((item) => {
      item.addEventListener("click", () => {
        const target = item.dataset.tab || item.dataset.page;
        if (!target) return;
        qsa(".panel").forEach((panel) => panel.classList.remove("active"));
  ...
  function init() {
    ...
    const sidebar = qs("#sidebarSnapshotText");
    if (sidebar) {
      sidebar.textContent = `떡잎마을 방범대 · ${META.dataRange}`;
    }

    try { renderMarketPage(); } catch (error) { console.warn("Page 1 렌더링 생략:", error); }
    try { renderSearchPage(); } catch (error) { console.warn("Page 2 렌더링 생략:", error); }
    try { renderReviewPage(); } catch (error) { console.warn("Page 3 렌더링 생략:", error); }
    try { renderAlertPage(); } catch (error) { console.warn("Page 4 렌더링 생략:", error); }
    ...
    try { renderChartsForPanel("A"); } catch (error) { console.warn("차트 렌더링 생략:", error); }

    loadDatalabData().catch((error) => {
      console.warn("DataLab 데이터 로딩 생략:", error);
    });
  }
