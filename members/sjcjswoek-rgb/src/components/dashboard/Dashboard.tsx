...
export default function Dashboard() {
  ...
  const [selectedAgentInsight, setSelectedAgentInsight] = useState<any | null>(null);
  const [openedInsightDetail, setOpenedInsightDetail] = useState<string | null>(null);
  const [agentStrategyState, setAgentStrategyState] = useState<{
  ...
  const agentSuggestionQuestions = [
    "📄 신제품 기획안 생성해줘",
    "🎯 이번 주 MD 액션 플랜 정리해줘",
    "🔥 이번 주 신규 진입 추천 성분 알려줘",
    "⚠️ 공급 과잉 위험 성분 우선순위 알려줘",
    "💬 부정 리뷰 많은 성분 개선 방향 알려줘",
    "📈 검색량 급상승 성분 기반 상세페이지 전략 추천해줘",
  ];

  const getLocalAgentFallback = (question: string) => {
    const normalized = question.replace(/\s+/g, "");

    if (normalized.includes("신제품") || normalized.includes("기획안") || normalized.includes("기획서") || normalized.includes("신규진입")) {
      return {
        id: "agent-new-product-plan",
        title: "MD 상품 기획서 초안",
        level: "기획 우선",
        summary:
          "검색량 상승, 리뷰 흐름, 수요-공급 신호, 경보 데이터를 종합해 MD가 빠르게 검토할 수 있는 신제품 기획 방향을 정리합니다.",
        evidence:
          "신제품 기회 경보 + 검색 관심도 상승 + 리뷰 이슈 + 수요-공급 기회 신호",
        strategy:
          "상승 성분을 핵심 효능으로 설정하고, 사용감·저자극·가격/리뷰 이슈를 보완한 상품 콘셉트로 전개합니다.",
        targetTitle: "신제품 기획 실행안",
        issues: [
          "상승 성분 선점 필요",
          "리뷰 불만 개선 포인트 반영 필요",
          "판매 채널용 차별화 메시지 필요",
        ...
        directions: [
          "4페이지 신제품 기회 후보 성분을 우선 검토합니다.",
          "2페이지 검색 트렌드 상승 키워드를 상세페이지 첫 화면에 반영합니다.",
          "3페이지 리뷰 이슈를 참고해 자극감·흡수력·사용감 보완 메시지를 구성합니다.",
          "경쟁 제품의 가격대와 리뷰 반응을 함께 확인해 상품 포지셔닝을 조정합니다.",
        ],
        actions: [
          "상품 콘셉트: 성분 효능 + 저자극 사용감 중심",
          "상세페이지: 검색 상승 근거와 리뷰 니즈를 카드형 콘텐츠로 배치",
          "프로모션: 출시 초기 체험단 운영 → 리뷰 확보 → 성분 키워드 광고 확장",
          "리스크 관리: 자극감·건조감 관련 FAQ와 사용 가이드를 상세페이지에 선제 배치",
        ],
        detailSections: [
          {
            title: "관련 데이터 및 참고 자료",
            items: [
              "1페이지 수요-공급 매트릭스: 신규 진입 가능성 확인",
              "2페이지 검색 트렌드: 성분 관심도 상승 여부 확인",
              "3페이지 리뷰 감성 분석: 부정 키워드 및 개선 포인트 확인",
              "4페이지 경보 알림 서비스: 신제품 기회/재고 리스크/부정 리뷰 경보 확인",
            ],
          },
          {
            title: "문서 활용 방법",
            items: [
              "초안은 MD 내부 검토용으로 사용합니다.",
              "실제 상품화 전에는 가격대, 경쟁 제품, 원가, 채널 적합성 검토가 필요합니다.",
              "리뷰 기반 불만 요소는 상세페이지와 제품 개발 요청 사항으로 분리해 관리합니다.",
            ],
          },
        ],
        downloadable: true,
        downloadTitle: "MD_상품_기획서_초안",
      };
    }

    if (normalized.includes("액션플랜") || normalized.includes("액션")) {
      return {
        id: "agent-weekly-action-plan",
        title: "주간 MD 액션 플랜 초안",
        level: "실행 우선",
        summary:
          "이번 주 성분 트렌드, 리뷰 이슈, 경보 데이터를 바탕으로 MD가 우선 실행해야 할 기획·재고·리뷰 개선 액션을 정리합니다.",
        evidence:
          "성분 트렌드 + 검색 추세 + 리뷰 분석 + 경보 데이터 종합",
        strategy:
          "신제품 후보 검토, 과공급 성분 소진, 부정 리뷰 개선을 동시에 관리하는 주간 실행안으로 전개합니다.",
        targetTitle: "주간 실행 우선순위",
        issues: ["기획 후보 선별", "재고 리스크 관리", "리뷰 리스크 대응"],
        directions: [
          "신제품 기획 후보 성분은 상품화 가능성과 경쟁 강도를 함께 검토합니다.",
          "과공급 성분은 재고 소진 프로모션 우선순위를 설정합니다.",
          "부정 리뷰 성분은 상세페이지 개선과 제품 개선 요청 사항으로 분리합니다.",
        ],
        actions: [
          "월요일: 경보 성분 리스트 확인 및 우선순위 선정",
          "수요일: 상세페이지/프로모션 초안 작성",
          "금요일: 검색량·리뷰·재고 변화 모니터링",
        ],
        detailSections: [
          {
            title: "이번 주 실행 순서",
            items: [
              "우선순위 1: 신제품 기획 후보 성분 검토",
              "우선순위 2: 재고 리스크 성분 프로모션 설계",
              "우선순위 3: 부정 리뷰 성분 상세페이지 개선",
            ],
          },
          {
            title: "성과 확인 지표",
            items: ["검색량 증감률", "재고 회전율", "부정 리뷰 비율", "상세페이지 전환율"],
          },
        ],
        downloadable: true,
        downloadTitle: "주간_MD_액션플랜_초안",
      };
    }

    if (normalized.includes("재고") || normalized.includes("과공급") || normalized.includes("공급과잉")) {
      return {
        id: "agent-inventory-risk",
        title: "공급 과잉 위험 성분 대응 전략",
        level: "주의",
        summary:
          "공급 과잉 영역의 성분은 신규 발주보다 재고 회전율 개선과 프로모션 설계가 우선입니다.",
        evidence: "4페이지 재고 리스크 경보 + 1페이지 수요-공급 매트릭스",
        strategy:
          "단순 할인보다 인기 성분과 묶은 루틴형 세트 구성, 단기 기획전, 증정 전략을 병행합니다.",
        targetTitle: "재고 리스크 대응",
        issues: ["공급 과잉", "가격 경쟁 위험", "소비자 관심도 둔화"],
        directions: [
          "공급 과잉 성분은 신규 발주보다 재고 소진 캠페인 우선순위를 높입니다.",
          "인기 상승 성분과 묶어 루틴형 세트 상품으로 구성합니다.",
          "가격 할인만 강조하지 않고 사용 상황·피부 고민 해결 메시지를 함께 제시합니다.",
        ],
        actions: [
          "단기 기획전 또는 1+1/증정 프로모션 검토",
          "상세페이지에 루틴 사용법과 병행 성분 조합 콘텐츠 추가",
          "재고 회전율과 검색량 추이를 주간 단위로 모니터링",
        ],
        detailSections: [
          {
            title: "재고 소진 아이디어",
            items: ["묶음 세트 구성", "인기 성분과 조합한 루틴 판매", "할인보다 체험/증정 중심 운영"],
          },
        ],
        downloadable: false,
      };
    ...
    if (normalized.includes("부정") || normalized.includes("리뷰") || normalized.includes("개선")) {
      return {
        id: "agent-negative-review",
        title: "부정 리뷰 이슈 성분 개선 전략",
        level: "개선 필요",
        summary:
          "부정 리뷰가 많은 성분은 제품 개선뿐 아니라 상세페이지 기대치 조정과 사용법 안내가 함께 필요합니다.",
        evidence: "4페이지 부정 리뷰 경보 + 3페이지 부정 키워드 분석",
        strategy:
          "자극감·건조감·흡수력 등 반복 불만을 제품 개선 포인트와 커뮤니케이션 보완 포인트로 분리합니다.",
        targetTitle: "리뷰 리스크 개선안",
        issues: ["자극감 불만", "사용감 불만", "기대 효능 불일치"],
        directions: [
          "3페이지 부정 키워드 상위 항목을 FAQ와 상세페이지 주의사항에 반영합니다.",
          "제형 개선이 어려운 경우 사용 순서·권장 빈도·병행 성분 안내를 강화합니다.",
          "긍정 리뷰 키워드와 함께 노출해 단점을 보완하는 메시지를 구성합니다.",
        ],
        actions: [
          "부정 키워드 TOP5 기반 QA 문구 작성",
          "상세페이지에 사용 가이드와 민감 피부 안내 추가",
          "리뷰 이벤트 운영 후 개선 반응 추적",
        ],
        detailSections: [
          {
            title: "상세페이지 보완 포인트",
            items: ["사용 빈도 안내", "민감 피부 주의 문구", "병행 사용 추천 성분"],
          },
        ],
        downloadable: false,
      };
    ...
    return {
      id: "agent-search-trend",
      title: "검색량 급상승 성분 활용 전략",
      level: "관찰 우선",
      summary: "검색량이 빠르게 증가한 성분은 콘텐츠 선점과 상품 기획 후보 검토가 필요합니다.",
      evidence: "2페이지 검색 트렌드 + 1페이지 성분 인기 순위",
      strategy: "검색 상승 성분을 상세페이지 키워드, 광고 소재, SNS 콘텐츠에 우선 반영합니다.",
      targetTitle: "급상승 성분 활용 전략",
      issues: ["검색 관심 증가", "콘텐츠 선점 필요", "경쟁 진입 가능성"],
      directions: [
        "검색량 상승 성분을 광고 문구와 상품명 후보에 반영합니다.",
        "연령대별 피부 고민과 연결해 타겟 메시지를 세분화합니다.",
        "경쟁 제품 수와 가격대를 함께 확인해 진입 가능성을 판단합니다.",
      ],
      actions: [
        "급상승 성분 카드뉴스 제작",
        "상품 상세페이지 SEO 키워드 반영",
      ...
      detailSections: [
        {
          title: "콘텐츠 소재",
          items: ["왜 지금 이 성분이 뜨는가", "어떤 피부 고민에 적합한가", "기존 인기 성분과 무엇이 다른가"],
        },
      ],
      downloadable: false,
    };
  ...
  const buildAgentDocumentHtml = (item: any) => {
    ...
    return `
      ...
      <h2>2. 기획 배경 및 목적</h2>
      <p>
        최근 검색량 상승, 소비자 리뷰 흐름, 경보 데이터 및 시장 트렌드 분석 결과를 기반으로
        현재 성분/상품군에 대한 시장 관심도가 높아지고 있습니다.
        다만 일부 부정 리뷰 및 공급 과잉 위험 신호도 함께 관찰되어,
        이를 보완한 상품 기획 전략 검토가 필요한 상황입니다.
      </p>

      <h2>3. 현재 시장 문제점 및 이슈</h2>
      <div class="box">
        <ul>
          ${(item.issues || []).map((issue: string) => `<li>${issue}</li>`).join("")}
        </ul>
      </div>
      ...
      <div class="box">
        <ul>
          ${(item.directions || []).map((direction: string) => `<li>${direction}</li>`).join("")}
        </ul>
      </div>
      ...
      ${detailSections.map((section: any) => `
        ...
        <div class="box">
          <ul>
            ${(section.items || []).map((detail: string) => `<li>${detail}</li>`).join("")}
          </ul>
        </div>
      `).join("")}
      ...
      <div class="conclusion-box">
        <p>
          현재 데이터 기준으로는 상품 기획 검토 가치가 충분하며,
          검색량·리뷰·경보 데이터를 종합했을 때 시장 반응 가능성이 확인되고 있습니다.
        </p>
        <p>
          다만 소비자 리뷰 기반 불만 요소와 시장 공급 상황을 함께 고려하여,
          단순 성분 중심 접근보다 사용감·저자극·상세페이지 메시지 전략까지 포함한
          통합 기획 방향으로 전개하는 것이 적합합니다.
        </p>
        <p>
          이후 리뷰 확보 → 검색량 추이 모니터링 → 광고 및 콘텐츠 확장 순서로
          단계적 운영 전략을 추진하는 방향이 권장됩니다.
        </p>
      </div>

      <div class="footer-note">
        본 문서는 AI Assistant 기반 자동 생성 초안이며, 실제 상품 기획 및 운영 전 MD 검토가 필요합니다.
      </div>
    `;
  };

  const downloadAgentDocument = (item: any) => {
    const title = item.downloadTitle || item.title.replace(/\s+/g, "_");
    const html = `
      <html>
        <head>
          <meta charset="utf-8" />
  ...
  const runAgentStrategy = (question: string) => {
    const normalizedQuestion = question.trim() || "🎯 이번 주 MD 액션 플랜 정리해줘";
    const fallback = getLocalAgentFallback(normalizedQuestion);

    setAgentPrompt(normalizedQuestion);
    setSelectedAgentInsight(fallback);
    setOpenedInsightDetail(null);
    setAgentStrategyState({
      status: "ready",
      source: "5페이지 로컬 전략 템플릿",
      error: "",
    });
  };

  const activeAgentInsights = selectedAgentInsight ? [selectedAgentInsight] : data.page5.insights;
  const activeTargetStrategy = selectedAgentInsight || data.page5.targetStrategy;
              ...
              <div className="agent-main-grid">
                <section className="agent-left-column">
                    ...
                    <div className="agent-input-card">
                      ...
                      <div className="agent-input-row">
                        ...
                        <button
                          ...
                          type="button"
                          aria-label="전략 제안 받기"
                          onClick={() => runAgentStrategy(agentPrompt)}
                        >
                          ➤
                        </button>
                      </div>
                  ...
                  <section className="card">
                    ...
                    {agentStrategyState.source ? (
                      <div className="agent-status-note">
                        데이터 소스: {agentStrategyState.source}
                      </div>
                    ) : null}
                    <div className="recommendation-grid">
                      {activeAgentInsights.map((item) => (
                        <article className="recommendation-card" key={item.id}>
                          ...
                          <div className="strategy-button-row">
                            <button
                              className="btn btn-outline strategy-button"
                              type="button"
                              onClick={() =>
                                setOpenedInsightDetail(openedInsightDetail === item.id ? null : item.id)
                              }
                            >
                              전략 상세 보기
                            </button>
                            ...
                            {item.downloadable ? (
                              <button
                                className="btn btn-primary strategy-download-button"
                                type="button"
                                onClick={() => downloadAgentDocument(item)}
                              >
                                Word 초안 다운로드
                              </button>
                            ) : null}
                          ...
                          {openedInsightDetail === item.id ? (
                            <div className="insight-detail-panel">
                              {(item.detailSections || []).map((section: any) => (
                                <div className="detail-section" key={section.title}>
                                  <strong>{section.title}</strong>
                                  <ul>
                                    {(section.items || []).map((detail: string) => (
                ...
                <aside className="agent-side-column">
                  <section className="card">
                    ...
                    <div className="suggestion-list">
                      {agentSuggestionQuestions.map((suggestion) => (
                        <button
                          className="question-chip"
                          type="button"
                          key={suggestion}
                          onClick={() => {
                            setAgentPrompt(suggestion);
                            runAgentStrategy(suggestion);
                          }}
                        >
                          {suggestion}
                        </button>
                  ...
                  <section className="card target-strategy-card">
                    ...
                    <div className="target-strategy">
                      <h2>{activeTargetStrategy.targetTitle || activeTargetStrategy.title}</h2>
                      <div className="strategy-block">
                        <span>부정 포인트</span>
                        <div className="strategy-tags">
                          {(activeTargetStrategy.issues || []).map((item: string) => <em key={item}>{item}</em>)}
                        </div>
                      </div>
                      <div className="strategy-block">
                        <span>제안 방향</span>
                        <ul>{(activeTargetStrategy.directions || []).map((item: string) => <li key={item}>{item}</li>)}</ul>
                      </div>
                      <div className="strategy-block">
                        <span>추천 액션</span>
                        <ul>{(activeTargetStrategy.actions || []).map((item: string) => <li key={item}>{item}</li>)}</ul>
                      </div>
                    </div>
                  </section>
                </aside>
