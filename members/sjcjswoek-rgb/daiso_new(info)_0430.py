...
OUTPUT_CSV = f"daiso_신상순(info)_{RUN_DATE}.csv"
OUTPUT_XLSX = f"daiso_신상순(info)_{RUN_DATE}.xlsx"
...
def extract_volume_ml(product_name):
    """
    상품명에서 단일상품 ml 용량만 추출.
    예: '본셉 비타씨 휩 퍼스트 에센스 100 ml' -> 100
    """
    if not product_name:
        return ""
...
def get_new_products(page, target_count=24):
    page.goto(CATEGORY_URL, wait_until="networkidle", timeout=60000)
    page.wait_for_timeout(4000)

    # 신상품순 버튼 클릭
    try:
        page.locator("span.name", has_text="신상품순").click()
        page.wait_for_timeout(4000)
...
def main():
    ...
    with sync_playwright() as p:
        ...
        products = get_new_products(page, TARGET_COUNT)
